...
__global__ void kernelAdd(float *d_a, float *d_b, float *d_c) {
  int id = threadIdx.x;
...
void CudaAdd(int n_elem, float *h_a, float *h_b, float *gpu_ref) {
  ...
  dim3 block(n_elem);
  dim3 grid(n_elem / block.x);
  printf("\nelem num: %d, block:(%d, %d, %d), grid:(%d, %d, %d)\n",
         n_elem, block.x, block.y, block.z, grid.x, grid.y, grid.z);

  kernelAdd<<<grid, block>>>(d_a, d_b, d_c);
...
int main() {
  ...
  // Compare results.
  printf("\nDoes CUDA result equals to CPU? %s.",
