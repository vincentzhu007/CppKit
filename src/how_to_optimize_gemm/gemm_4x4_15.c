...
static void pack_4xk_a(int k, float *a, int lda, float *dst_a) {
...
static void pack_kx4_b(int k, float *b, int ldb, float *dst_b) {
  float *b_j0_ptr = &B(0, 0);
  float *b_j1_ptr = &B(1, 0);
  float *b_j2_ptr = &B(2, 0);
  float *b_j3_ptr = &B(3, 0);
...
static void inner_kernel(int m, int n, int k, float *a, int lda, float *b,
                         int ldb, float *c, int ldc, int is_first_time) {
  assert(m % 4 == 0);
  assert(n % 4 == 0);
  ...
  float *tmp_b = (float *)malloc(k * n * sizeof(float));
  assert(tmp_b != NULL);

  for (int j = 0; j < n; j += 4) {
    if (is_first_time) {
      pack_kx4_b(k, &B(0, j), ldb, &tmp_b[j * k]);
    }

    for (int i = 0; i < m; i += 4) {
      if (j == 0) {
        pack_4xk_a(k, &A(i, 0), lda, &tmp_a[i * k]);
      }
